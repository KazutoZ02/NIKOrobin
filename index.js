// index.js — main bot + Express dashboard (render-friendly)
...
if (!TOKEN || !CLIENT_ID || !GUILD_ID) {
  console.error('Missing env: DISCORD_TOKEN, CLIENT_ID and GUILD_ID are required.');
  process.exit(1);
}
...
client.on('guildMemberAdd', async (member) => {
  ...
  try {
    ...
    if (channel && fs.existsSync(path.join(__dirname, cfg.welcomeImage))) {
      await channel.send({ content: `Welcome <@${member.id}>!`, files: [path.join(__dirname, cfg.welcomeImage)] }).catch(console.error);
    }
...
// Interaction handling -- delegate to commands
client.on('interactionCreate', async (interaction) => {
  ...
  if (interaction.isChatInputCommand()) {
    ...
    try { await command.execute(interaction, client, cfg, readConfig); } catch (err) { console.error(err); interaction.reply({ content: 'Error executing command.', ephemeral: true }); }
  }
...
// Express dashboard and health endpoint (Render requires a web server)
const app = express();
...
// health check root
app.get('/', (req, res) => {
  res.send('Niko Robin Bot is running 🌸');
});
...
client.login(TOKEN).catch(err => {
  console.error('Failed to login the bot. Check DISCORD_TOKEN and internet connectivity.', err);
  process.exit(1);
});
